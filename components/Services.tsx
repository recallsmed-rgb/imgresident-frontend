...
const PricingCard: React.FC<PricingCardProps> = ({ plan, onCtaClick }) => {
  ...
  const cardClasses = [
    'bg-white rounded-2xl p-8 shadow-lg hover:shadow-2xl transition-all duration-300 flex flex-col relative',
    highlighted ? 'border-2 border-primary' : 'border border-slate-100',
  ].join(' ');

  const buttonClasses = highlighted
    ? 'w-full py-3 bg-slate-900 text-white font-bold rounded-xl hover:bg-slate-800 transition-colors flex justify-center items-center'
  ...
  return (
    <div className={cardClasses}>
      ...
      <div className="mb-6 flex items-baseline">
        ...
        {priceSuffix ? (
          <span className="text-slate-500 font-medium ml-2">{priceSuffix}</span>
        ) : null}
      </div>
    ...
    <section id="services" className="py-24 bg-slate-50 relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-lg text-slate-600">Your complete pathway to a U.S. Residency</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
          {services.map((service) => {
            ...
            return (
              <div
                key={service.title}
                className="bg-white p-6 rounded-xl border border-slate-200 hover:border-primary/50 hover:shadow-lg transition-all group cursor-default"
              >
                <div className="w-14 h-14 bg-slate-100 rounded-lg flex items-center justify-center text-primary mb-6 group-hover:bg-primary group-hover:text-white transition-colors">
